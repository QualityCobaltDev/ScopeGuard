...
export function SiteFooter({ site }: { site: SiteContent }) {
  ...
  return (
    <footer className="mt-16 border-t border-border/60 bg-[#050a16] sm:mt-20 md:mt-24">
      <Reveal className="container" y={20} duration={0.62}>
        <div className="grid gap-8 py-12 sm:py-14 md:grid-cols-4 md:gap-10 md:py-16">
          ...
          <div>
            ...
            <ul className="mt-3 space-y-1.5 text-sm text-muted sm:mt-4">
              {site.footer.companyLinks.map((item) => (
                <li key={item.href}>
                  <Link className="inline-block rounded-md py-1 transition duration-300 hover:text-foreground" href={item.href}>{localizeText(item.label)}</Link>
                </li>
              ))}
            </ul>
          </div>
          <div>
            ...
            <ul className="mt-3 space-y-1.5 text-sm text-muted sm:mt-4">
              ...
              <li>
                <a className="inline-block rounded-md py-1 transition duration-300 hover:text-foreground" href={`mailto:${site.contactEmail}`}>{site.contactEmail}</a>
              </li>
            </ul>
          </div>
        </div>
