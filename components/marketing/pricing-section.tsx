...
export function PricingSection({ tiers }: { tiers: PricingTier[] }) {
  return (
    <section className="container py-20" id="pricing">
      <SectionTitle eyebrow="Offer Ladder" title="Choose your growth level" description="Every tier is editable in admin and designed for premium conversion flow." />
      <div className="grid gap-6 lg:grid-cols-3">
        {tiers.map((tier) => (
          <Card key={tier.id} className={`p-6 ${tier.featured ? "border-brand/60 bg-gradient-to-b from-brand/10 via-card/90 to-card/80 shadow-glow" : ""}`}>
