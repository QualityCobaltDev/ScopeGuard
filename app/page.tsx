  ...
  return (
    <>
      ...
      <section className="container py-8">
        <SectionTitle eyebrow="Pain points" title="Freelancers lose margin in the same 3 places" description="Weak agreements, uncontrolled scope, and fragmented communication quietly erode profit." />
      ...
      <section className="container py-20">
        <SectionTitle eyebrow="Solution" title="A complete freelancer operating system" description="ScopeGuard combines legal protection with execution assets so your process feels authoritative from day one." />
      ...
      <section className="container py-20"><SectionTitle eyebrow="Testimonials" title="Proof from freelancers using ScopeGuard" /><div className="grid gap-5 md:grid-cols-3">{testimonials.map((item) => <Card key={item.id} className="p-6"><p className="text-sm leading-7 text-muted">“{item.quote}”</p><p className="mt-6 font-medium text-foreground">{item.name}</p><p className="text-xs text-muted">{item.role}</p></Card>)}</div></section>
