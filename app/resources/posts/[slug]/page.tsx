      ...
      <div className="mt-6 grid gap-4 sm:mt-8 md:grid-cols-2">
        {linkedResources.map((resource) => (
          <Card key={resource.id} className="p-5">
            ...
            <p className="mt-2 text-sm leading-7 text-muted">{resource.summary}</p>
            <a className="mt-3 inline-flex min-h-10 items-center text-sm underline" href={`/api/resources/download/${resource.id}`}>{resource.ctaLabel || "Download"}</a>
          </Card>
        ))}
      </div>
    </div>
